...
class ProPublicaAPI(object):
    ...
    def __init__(self, api_key):
        ...
        self.session.headers.update(
            {"X-API-Key": self.api_key, "cache-control": "no-cache"}
        )
    ...
    def bills_search(self, query_text):
        ...
        """
        query_url = os.path.join(ProPublicaAPI.API_ROOT, "bills", "search.json")
        params = {"query": '"' + query_text + '"'}

        response = self.session.get(query_url, params=params)
        response.raise_for_status()
        response_json = response.json()
        if response_json["results"][0]["offset"] > 0:
            raise NotImplementedError("Pagination not enabled. ")
