...
@click.option(
    "--parcel_table",
    default="padus2_0fee",
    help="Tablename in lands sqlite database containing parcels",
    show_default=True,
)
@click.option(
    "--parcel_col",
    default="unit_nm",
    help="Column in parcel sqlite database containing unique identifier names.",
    show_default=True,
)
@click.option("--api-key", help="ProPublica Congress API key.", required=True)
@click.option(
    "--overwrite", help="Overwrite existing bills database", is_flag=True
)
@click.option("--debug", is_flag=True)
@click.option(
    "--parcels",
    metavar="<file>",
    help="File containing specific parcel names (ignores DB.) Useful for resuming failed parcels.",
)
@click.pass_context
def download(ctx, **args):
    ...
    if args["parcels"]:
        ...
        logger.info(
            f"Found {len(parcels)} unique parcels in {args['parcel_table']}.{args['parcel_col']}"
        )
