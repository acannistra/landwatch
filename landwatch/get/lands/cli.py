...
@lands.command(help="Download public lands data.")
# @click.argument("type")
@click.argument("dest")
@click.pass_context
def download(ctx, dest):
    ...
    if ctx.obj["type"] == "usgs":
        ...
        raise click.UsageError(
            f"Data type \"{ctx.obj['type']}\" is not supported."
        )
    _data.download(dest)
...
@click.option(
    "-d",
    "--destination",
    help="Destination. Defaults to original source location.",
)
@click.option(
    "--all",
    help="Output entire PAD-US (if not given, output only contains federally-managed land)",
)
@click.pass_context
